...
class MenuAndHistoryTests(TestCase):
    ...
    def test_sidebar_labels_and_history_route(self):
        ...
        self.assertContains(system, reverse("system_section", kwargs={"key": "molds"}))
        self.assertNotContains(system, "<h2>مدیریت داده‌های پایه سامانه</h2>")

        molds = self.client.get(reverse("system_section", kwargs={"key": "molds"}))
        self.assertEqual(molds.status_code, 200)
        self.assertContains(molds, "انواع قالب")
        ...
        self.assertContains(products, "data-group-select")
