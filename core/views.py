...
from catalog.models import Product, ProductionUnit, StoppageReason
...
def dashboard(request):
    ...
    context = {
        ...
        "pending_plans": WeeklyPlan.objects.filter(
            status=WeeklyPlan.Status.PENDING
        ).count(),
        "unit_bars": unit_bars,
...
def _fitting_rows(qs):
    return [
        [
            str(r.date),
            f"واحد {r.unit.number}",
            str(r.machine),
            r.product.name,
            r.shot_cycle,
            r.produced_quantity,
...
def reports(request):
    ...
    if report_type == "pipe":
        qs = PipeProduction.objects.select_related("unit", "line").all()
        if keyword:
            qs = qs.filter(pipe_type__icontains=keyword)
        headers = REPORT_PIPE_HEADERS
    else:
        report_type = "fitting"
        qs = FittingProduction.objects.select_related("unit", "machine", "product").all()
        if keyword:
            qs = qs.filter(product__name__icontains=keyword)
        headers = REPORT_FITTING_HEADERS
    ...
    rows = _pipe_rows(qs) if report_type == "pipe" else _fitting_rows(qs)
    ...
    context = {
        ...
        "total_produced": sum(
            (r.produced_quantity for r in qs), 0
        ),
    }
